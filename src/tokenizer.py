...
class TokenizerBuilder():
    ...
    def __init__(self, lang, other_lang=None):
        ...
        if not os.path.exists(self.tokenizer_path) and other_lang is None:
            raise Exception('You are trying to use a tokenizer from a baseline model that has not been trained yet. '
              'First, train a baseline model which will train and save a tokenizer, which then will be loaded here.')
